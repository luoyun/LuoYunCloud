...
static int loglevel = LYDEBUG;
static FILE *LOGFH = NULL;
static char logFile[MAX_PATH];
static pthread_mutex_t log_mutex = PTHREAD_MUTEX_INITIALIZER;

static void (* __callback)(int type, char * msg, int data) = NULL;
static int __callback_data;
...
/*
** logging with priority check and message tag(time+level)
** log file rotation is enforced, totoally 5 rotated files are allowed
*/
...
int logerror(const char *format, ...)
{
    ...
    if (__callback) {
        ...
        va_start(ap, format);
        char s[LOG_CALLBACK_MSG_MAX] = {'\0', };
        if (vsnprintf(s, LOG_CALLBACK_MSG_MAX, format, ap) >=
            LOG_CALLBACK_MSG_MAX)
            s[LOG_CALLBACK_MSG_MAX-1] = '\0';
        __callback(LYERROR, s, __callback_data);
        va_end(ap);
...
int logwarn(const char *format, ...)
{
    ...
    if (__callback) {
        ...
        if (vsnprintf(s, LOG_CALLBACK_MSG_MAX, format, ap) >=
            LOG_CALLBACK_MSG_MAX)
            s[LOG_CALLBACK_MSG_MAX-1] = '\0';
        __callback(LYWARN, s, __callback_data);
        va_end(ap);
        pthread_mutex_unlock(&cb_mutex);
    }
