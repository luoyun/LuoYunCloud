...
class WikiManagement(RequestHandler):
    ...
    def prepare(self):
        ...
        if topic_id:
            self.topic = self.db.query(Topic).get( topic_id )
            if not self.topic:
                self.write( self.trans(_('No such topic : %s')) % topic_id )
                return self.finish()

        c_id = self.get_argument('catalog', 0)
        self.catalog = self.db.query(WikiCatalog).get(c_id)
    ...
    def get_index(self):
        catalogs = self.db.query(WikiCatalog).all()
